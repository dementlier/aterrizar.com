...
class FriendServiceTest {
	...
	def void setUp() {
		...
		user6 = new User("Carlos", "Calvo", "cc", "cc@p.com", new Date(1), "1234", false)
		user7 = new User("Nadie", "Nadie", "nn", "nn@p.com", new Date(1), "1234", false)
		...
		friendService.addUser(user6)
		friendService.addUser(user7)
	...
	def testUsersConectedToUser1() {
		...
		friendService.befriend(user5, user6)
		var connectedUsers = friendService.getConnectedUsers(user)
		assertEquals(5, connectedUsers.size())
		...
		assertTrue(connectedUsers.exists[u|u.username == user6.username])
	}
	...
	def void tearDown() {
		...
		friendService.deleteUser(user6)
		friendService.deleteUser(user7)
	}
